...
public class TitaniumDensityFunctionTypes {
    ...
    private static MapCodec<? extends DensityFunction> densityFunctionType(String path, KeyDispatchDataCodec<? extends DensityFunction> dispatch) {
        return Registry.register(BuiltInRegistries.DENSITY_FUNCTION_TYPE, Titanium.id(path), dispatch.codec());
    }
    ...
    record DistanceFromCenterGradient(int minDistance, int maxDistance, double fromValue,
                                      double toValue) implements DensityFunction.SimpleFunction {
