...
public class TitaniumItemGroups {
	...
	public static final ResourceKey<CreativeModeTab> ITEM_GROUP_KEY = ResourceKey.create(BuiltInRegistries.CREATIVE_MODE_TAB.key(), Titanium.id("titanium"));
	public static final CreativeModeTab ITEM_GROUP = FabricItemGroup.builder()
			.icon(TitaniumBlocks.SOD.asItem()::getDefaultInstance)
			.title(Component.translatable("itemGroup.titanium"))
			.build();
	...
	public static void initialize() {
		...
		ItemGroupEvents.modifyEntriesEvent(ITEM_GROUP_KEY)
				.register(itemGroup -> itemGroup.acceptAll(toItemStackCollection(
						CLART, SOD, SOD_BATCH,
						...
						VOLLON_ALVEOLI, VOLLON_LIGAMENTS
				)));
		//@formatter:on
	...
	private static Collection<ItemStack> toItemStackCollection(ItemLike... itemConvertibles) {
